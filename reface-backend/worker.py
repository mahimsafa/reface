...
class ImageProcessingWorker:
        ...
        while retries < self.max_retries:
            try:
                ...
                if retries >= self.max_retries:
                    logger.error(
                        f"Failed to connect to RabbitMQ after {self.max_retries} attempts: {e}")
                    return False
    ...
    def update_process_status(self, process_id, status, result_image_path=None):
        ...
        files = {
            # ensure status is always in multipart form-data
            'status': (None, status)
        }
        ...
        try:
            ...
            response.raise_for_status()
            logger.info(
                f"Successfully updated process {process_id} status to {status}")
            return True
        except requests.RequestException as e:
            logger.error(
                f"Failed to update process {process_id} status: {str(e)}")
            return False
        ...
        try:
            ...
            try:
                if not self.update_process_status(process_id, 'processing'):
                    logger.error(
                        f"Failed to update process {process_id} status")
                ...
                logger.info(
                    f"Successfully processed process {process_id}. Result saved to: {output_path}")
                ...
                logger.error(
                    f"Error processing message {process_id}: {str(e)}")
                # Try to update status to failed
                try:
                    self.update_process_status(process_id, 'failed')
                except Exception as update_err:
                    logger.error(
                        f"Failed to update process {process_id} status to failed: {str(update_err)}")
