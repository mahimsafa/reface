...
export async function listProcessRecords(
  userId: number,
  options: PaginationOptions & { status?: string } = {}
): Promise<PaginatedResult<ProcessRecord>> {
  ...
  try {
    ...
    // @ts-ignore
    countQuery = countQuery.where(eq(processRecords.userId, userId));
    // @ts-ignore
    query = query.where(eq(processRecords.userId, userId));
    ...
    const sortField =
      options.sortBy &&
      processRecords[options.sortBy as keyof typeof processRecords]
        ? options.sortBy
        : "createdAt";
...
export async function getProcessRecordById(userId: number, id: number) {
  // const rows = await db.select().from(processRecords).where(eq(processRecords.id, id));
  // return rows[0] || null;
  const record = await db.query.processRecords.findFirst({
    where: and(eq(processRecords.id, id), eq(processRecords.userId, userId)),
  });
  return record || null;
}
